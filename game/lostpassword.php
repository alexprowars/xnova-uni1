...
function sendnewpassword($id, $key){
	...
	if (!preg_match("/^[А-Яа-яЁёa-zA-Z0-9]+$/", $key)) {
		...
		$sucess = mail($mailto, "Новый пароль в Xnova Game", $body, $headers);
	...
	if ($step == 1 or $step == 0) {
		...
		if ($login != "") {
			...
			if ($inf['id'] != "") { 
				...
				Для восстановления пароля перейдите по ссылке: <a href='http://uni1.xnova.su/?set=lostpassword&id=".$inf['id']."&passw=".$key."'>http://uni1.xnova.su/lostpassword.php?id=".$inf['id']."&passw=".$key."</a>";
				$body = convert_cyr_string (stripslashes($body),w,w);
