...
if ( $s == 1 && isset( $_GET['bid'] ) ) {
	...
	if ( !$buddy ) {
		...
		message( $lang['Request_sent'], $lang['Buddy_request'], '?set=buddy' );
	} else {
...
if ( !isset( $a ) ) {
	$page .= "
	<tr>
		<th colspan=6><a href=?set=buddy&a=1>{$lang['Requests']}</a></th>
	</tr><tr>
		<th colspan=6><a href=?set=buddy&a=1&e=1>{$lang['My_requests']}</a></th>
	</tr><tr>
		<td class=c></td>
...
while ( $b = mysql_fetch_array( $buddyrow ) ) {
	...
	if ( $u["ally_id"] != 0 ) {
		$UserAlly = "<a href=?set=alliance&mode=ainfo&a=" . $u["ally_id"] . ">" . $u["ally_name"] . "</a>";
	} else
		$UserAlly = "";
	...
	if ( isset( $a ) && isset( $e ) ) {
		$UserCommand = "<a href=?set=buddy&s=1&bid=" . $b["id"] . ">{$lang['Delete_request']}</a>";
	} elseif ( isset( $a ) ) {
		$UserCommand = "<a href=?set=buddy&s=1&bid=" . $b["id"] . ">{$lang['Ok']}</a><br/>";
		$UserCommand .= "<a href=?set=buddy&a=1&s=1&bid=" . $b["id"] . ">{$lang['Reject']}</a></a>";
	} else {
		$UserCommand = "<a href=?set=buddy&s=1&bid=" . $b["id"] . ">{$lang['Delete']}</a>";
	}

	$page .= "
	<tr>
		<th width=20>" . $i . "</th>
		<th><a href=?set=messages&mode=write&id=" . $u["id"] . ">" . $u["username"] . "</a></th>
		<th>{$UserAlly}</th>
		<th><a href=\"?set=galaxy&mode=3&galaxy=" . $u["galaxy"] . "&system=" . $u["system"] . "\">" . $u["galaxy"] . ":" . $u["system"] . ":" . $u["planet"] . "</a></th>
		<th>{$LastOnline}</th>
		<th>{$UserCommand}</th>
	</tr>";
}
...
if ( $a == 1 ) {
	$page .= "
	<tr>
		<td colspan=6 class=c><a href=?set=buddy>{$lang['Back']}</a></td>
	</tr>";
}
