...
include('game/admin/statfunctions.php');
   ...
   if ($user['authlevel'] >= 3) {
	...
	$Spr_Online = doquery("SELECT * FROM {{table}} WHERE `onlinetime` < '{$Time_Online}' AND `onlinetime` > '0' AND `urlaubs_modus_time` = '0' AND `banaday` = '0' AND `deltime` = '0'", "users");
...
$GameUsers  = doquery("SELECT u.*, s.total_rank, s.tech_rank, s.fleet_rank FROM ({{table}}users u, {{table}}users_inf ui) LEFT JOIN {{table}}statpoints s ON s.id_owner = u.id AND s.stat_type = 1 WHERE ui.id = u.id AND u.authlevel < 3 AND (u.onlinetime > ui.register_time + 1800)", '');	// Удаляем статистику игроков
	...
	$Rank           = 1;
	$RankQry        = doquery("SELECT `id_owner` FROM {{table}} WHERE `stat_type` = '1' AND `stat_code` = '1' ORDER BY `tech_count` DESC;", 'statpoints');
	...
	while ($CurAlly = mysql_fetch_assoc($GameAllys)) {
		...
		$QrySumSelect  .= "FROM {{table}} WHERE `stat_type` = '1' AND `id_ally` = '". $CurAlly['id'] ."';";
	...
	// Чистим старые логи
	doquery ( "DELETE FROM {{table}} WHERE `message_time` <= '". (time() - 604800) ."';", 'messages');
	...
	message ( $Message, $lang['adm_stat_title'] );
