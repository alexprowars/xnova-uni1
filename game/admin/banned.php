...
if ($user['authlevel'] >= 1) {
	...
	if ($mode == 'banit') {
		...
		$userz = doquery("SELECT id FROM {{table}} WHERE username = '".$name."';", "users", true);
		if (!isset($userz['id']))
		...
		if ($_POST['ro'] == "1") {
			...
			doquery("UPDATE {{table}} SET `metal_mine_porcent` = '0', `crystal_mine_porcent` = '0', `deuterium_sintetizer_porcent` = '0', `solar_plant_porcent` = '0', `fusion_plant_porcent` = '0', `solar_satelit_porcent` = '0' WHERE `id_owner` = '".$userz['id']."'", "planets");
		}
	...
	display( $Page, $lang['adm_bn_ttle'], false, true, true);
} else {
