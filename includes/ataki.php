...
function calculateAttack (&$attackers, &$defenders, $injener) {
		...
		foreach ($defenders as $fleetID => $defender) {
			foreach ($defender['def'] as $element => $amount) {
				...
				if ($amount == 0)
					unset($defenders[$fleetID]['def'][$element]);
			}
		}
		...
		for ($round = 0, $rounds = array(); $round < $max_rounds; $round++) {
			...
			foreach ($attackers as $fleetID => $attacker) {
				...
				foreach ($attacker['detail'] as $element => $amount) {
					...
					$attackers[$fleetID]['techs'] = array(round($defTech, 2), round($shieldTech, 2), round($attTech, 2));
			...
			foreach ($attackers as $fleetID => $attacker) {
				$attackers[$fleetID]['detail'] = $attacker_n[$fleetID];
			}
			
			foreach ($defenders as $fleetID => $defender) {
				$defenders[$fleetID]['def'] = $defender_n[$fleetID];
			}
		}
		...
		foreach ($defenders as $fleetID => $defender) {
			foreach ($defender['def'] as $element => $amount) {
				if ($element < 300) {
					...
					$giveback = $lost * (rand($k - 3, $k + 3) / 100);
					$defenders[$fleetID]['def'][$element] += $giveback;

					if ($defenders[$fleetID]['def'][$element] < 1) {
						$r = rand(0, 1);
						if ($r == 0)
							$defenders[$fleetID]['def'][$element] = 0;
					}
				}
				$totalResourcePoints['defender'] -= $pricelist[$element]['metal'] * $amount ;
				$totalResourcePoints['defender'] -= $pricelist[$element]['crystal'] * $amount ;
